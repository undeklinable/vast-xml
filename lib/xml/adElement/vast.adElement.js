...
const VastCreativesElement = require('./creativesElement/vast.creativesElement');
...
class VastAdElement {
  ...
  configureAdByType() {
    const subElementType = VastAdElement.isWrapper(this._ad) ? this.setAdAsWrapper() : this.setAdAsInline();
    const vastCreativesElement = new VastCreativesElement(this._ad, this._track);

    vastCreativesElement.initCreatives(subElementType)
