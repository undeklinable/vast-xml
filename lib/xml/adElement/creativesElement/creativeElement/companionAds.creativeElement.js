...
class CompanionAdsCreativeElement extends BaseCreativeElement {
  ...
  insertCompanionAds() {
    ...
    this._creatives.map(companionAd => this._insertCompanionAd(companionAd));

    return this;
  }

  _insertCompanionAd(companionAd) {
    const companionXmlElement = this._xmlCompanionAdsElement.element(XML_CONSTANTS.companion, companionAd.attributes);
