...
const CreativesElement = require('./creativesElement/creativesElement');
...
class AdElement {
  ...
  insertImpressionTracker(xmlElement) {
    if (this._track) {
      this._ad.impressions.forEach(impression => {
        ...
        xmlElement.element('Impression', impressionAttrs).cdata(impression.url);
      });
    }
  ...
  setAdAsWrapper() {
    ...
    wrapperElement.element('AdSystem', { version : this._ad.adSystem.version || DEFAULT_AD_SYSTEM_VERSION })
  ...
  addExtensions(subElementType) {
    if (this._ad.extensions) {
      const extensions = subElementType.element('Extensions');
